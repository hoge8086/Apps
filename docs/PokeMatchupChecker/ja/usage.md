...
パーティ構築時に、環境でよく使われるポケモンに対抗できるポケモンがいるかを検討したり、対戦時に選出するポケモンを決めたりする際に活用できます。
...
| 5 | 素早さマーク | すばやさの種族値が高い側に三角を表示します。同速の場合は表示しません。 |

</div>

倍率は、技タイプと相手のタイプなどをもとに計算され、タイプ相性に関する一部の特性も考慮されます。タイプ一致ボーナス（STAB）は倍率に含まれません（将来的に、倍率へ含めるかどうかを選択できるようにする予定です）。<br>
...
[ダメージ計算画面](#damage-calculation)で組み合わせを「有利」に設定すると、味方パーティが有利に戦える敵ポケモンの一覧を相性表でまとめて網羅的に確認できます。
...
| 3 | 敵の確認 | 選択した味方のうち、1匹以上との組み合わせが「有利」に設定されている敵ポケモンは、列が緑色で表示されます。 |
