...
ポケ相性チェッカーは、ポケモン同士のタイプ相性を表形式で分かりやすくまとめて確認できるアプリです。<br><br>

表内の対戦するポケモンの組み合わせごとに、覚えているわざのなかで最もタイプ相性のよいわざのタイプと倍率を表示できます。<br>
味方から敵への攻撃だけでなく、敵から味方への攻撃のタイプ相性も確認できます。<br><br>

タイプ相性を覚えていない初心者でも、一目で有利に戦えるかどうかを判断できます。<br>
パーティ構築時、環境でよく使われるポケモンに対して、対抗できるポケモンがパーティにいるかを確認したり、対戦時のポケモン選出に活用することができます。

<div class="notice-box" markdown="1">

**注意事項**
」
- 「もちもの」や「天候」、「異常状態」などには現在未対応です。
- ダメージ計算において一部の特性・わざの効果には未対応です。
- 対応している特性・わざであれば正しくダメージ計算するよう実装しておりますが、それを保証するものではありません。
  正確な計算は他のサイト等をご利用ください。
- 現時点ではシングルバトルのみを想定したツールです。
